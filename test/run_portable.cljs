;; run_portable.cljs — run this suite on a second host.
;;
;;   nbb --classpath "src:test" test/run_portable.cljs
;;
;; Why this file exists. Every namespace in src/ is .cljc, the README calls
;; the library "portable .cljc", and deps.edn shipped exactly one runner:
;; `clojure -M:test`. So ClojureScript never ran an assertion, and three
;; reader conditionals — revrec/round-cents, leadscore/decay-multiplier and
;; leadscore/round-points — had a :cljs branch that nothing had executed.
;; That claim was not failing. It was unmeasured, and an unmeasured claim
;; and a measured, satisfied one produce the same output, which is nothing.
;;
;; Measured 2026-09-10, before this file existed: replacing
;; `:cljs (js/Math.round cents)` with `:cljs cents` in revrec — dropping
;; cent-rounding on one host only — left `clojure -M:test` green at 30/30,
;; because the :cljs branch is structurally invisible from the JVM. A second
;; host is the only thing that can tell those two trees apart.
;;
...
;; "0 failures, 0 errors" and exits 0, which is the same answer it gives
;; when everything really did pass.
;;
...
;; literal list is a claim about the tree, so it is checked against the
;; tree: every `*_test.cljc` under test/ must either be in the list or be
;; named as an exclusion, and every entry must actually report a result.
...
(ns run-portable
  (:require ["fs" :as fs]
            ...
            [clojure.test :as t]
            [kotoba.crm.funnel-test]
            [kotoba.crm.leadscore-test]
            [kotoba.crm.pipeline-test]
            [kotoba.crm.revrec-test]))
...
(def excluded
  ...
  Empty on purpose: this library has no dependencies at all (deps.edn's
  :deps is `{}`), so there is no namespace a second host cannot reach. An
  entry added here has to carry a reason a reader can check."
  '{})
...
(let [per-ns (map vector portable-namespaces @results)
  ...
  (cond
    ...
    ;; in the list with no file fails at `require` first, so the branch could
    ;; never be made to fire, and a check that cannot discriminate is no
    ;; cheaper to read than one that cannot pass.
    (seq unwired)
